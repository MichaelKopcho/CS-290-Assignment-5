...
// Create a twit using the twit handlebars template
function createNewTwit(twitText, twitAuthor) {
	// Pass the parameters into a templateContext object
	var templateContext = {
		twitText: twitText,
		twitAuthor: twitAuthor
	}
	
	// Generate the twit HTML using the templateContext
	var twitHtml = Handlebars.templates.twit(templateContext)
	console.log("== twitHtml:", twitHtml)
	
	return twitHtml
}
 
function insertNewTwit(twitText, twitAuthor) {
	// Create a new twit using the createNewTwit function
	var newTwit = createNewTwit(twitText, twitAuthor)

	// Insert the new twit into the twit container
